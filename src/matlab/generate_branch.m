...
% Allocate Space
x_race = zeros(1,Nrace+Nstraight);
y_race = zeros(1,Nrace+Nstraight);
...
%straight inner section on the top -- new tube top
for i = Nrace+1:Nrace+ceil(Nstraight/2)
    theta = (i-(Ncurve+ceil(Nstraight/2))-1)*dtheta-pi/2;
    theta1 = pi/2+(i-(Ncurve+ceil(Ncurve/2)+ceil(Nstraight/2)))*dtheta;
    y_race(1,i) = centery+0.05*(-cos(theta))*cos(theta1)+R2+R3+0.038;
    x_race(1,i) = centerx2-(i-Nrace)*ds;
    fprintf(vertex_fid, '%1.16e %1.16e\n', x_race(1,i), y_race(1,i));
end

%straight inner section on the top -- new tube bottom
for i = Nrace+ceil(Nstraight/2)+1:Nrace+Nstraight
    theta2 = (i-1)*dtheta-pi/2;
    theta3 = pi/2+(i-(ceil(Ncurve/2)+ceil(Nstraight/2))-1)*dtheta;
    y_race(1,i) = centery+R2-0.05*(-cos(theta2))*cos(theta3)+R3;
    x_race(1,i) = centerx2-(i-(Nrace+ceil(Nstraight/2)))*ds;
    fprintf(vertex_fid, '%1.16e %1.16e\n', x_race(1,i), y_race(1,i));
end
